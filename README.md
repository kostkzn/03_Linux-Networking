...
Type `sudo sysctl --system` command to reload `sysctl.conf` variables if you need to edit it
...
Network’s configuration files are placed in `/etc/sysconfig/network-scripts/`. File names have a format like `ifcfg-[network device name]`, e.g. `ifcfg-enp0s3`.  
